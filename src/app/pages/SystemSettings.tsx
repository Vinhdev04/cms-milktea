...
function Toggle({ defaultChecked = false, label, description }: ToggleProps) {
  ...
  return (
    <div className="flex items-center justify-between py-3.5 border-b last:border-0" style={{ borderColor: '#F0F7F3' }}>
      ...
      <button onClick={() => setChecked(!checked)}
        ...
        style={{ width: '40px', height: '22px', background: checked ? '#2D6A4F' : '#E5E7EB' }}>
  ...
  return (
    <div style={{ fontFamily: "'Be Vietnam Pro', sans-serif" }}>
      <div className="mb-6">
        ...
        <p style={{ fontSize: '13.5px', color: '#6B9080' }}>Quản lý và tùy chỉnh hệ thống SMYOU MilkTea CMS</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        {/* Sidebar Nav */}
        <div className="rounded-xl overflow-hidden"
          style={{ background: 'white', border: '0.5px solid #E0EDE6', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', height: 'fit-content' }}>
          {settingsSections.map((s, i) => (
            <button key={s.id} onClick={() => setActiveSection(s.id)}
              className="w-full flex items-center justify-between px-4 py-3.5 transition-all"
              style={{
                background: activeSection === s.id ? '#E8F5EC' : 'white',
                borderBottom: i < settingsSections.length - 1 ? '1px solid #F0F7F3' : 'none',
                color: activeSection === s.id ? '#2D6A4F' : '#6B9080'
              }}>
        ...
        <div className="lg:col-span-3 space-y-4">
              ...
              <div className="rounded-xl p-5"
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={f.label}>
                      <label style={{ fontSize: '12.5px', fontWeight: 600, color: '#6B9080', display: 'block', marginBottom: '5px' }}>{f.label}</label>
          ...
          {activeSection === 'appearance' && (
            <div className="rounded-xl p-5"
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  { label: 'Màu chủ đạo', value: '#2D6A4F' },
                  { label: 'Màu phụ', value: '#A8D5BA' },
                  { label: 'Màu nhấn', value: '#FCBABD' },
                  { label: 'Màu nền', value: '#E8F5EC' },
                ].map(c => (
                  <div key={c.label}>
                    <label style={{ fontSize: '12.5px', fontWeight: 600, color: '#6B9080', display: 'block', marginBottom: '5px' }}>{c.label}</label>
                    <div className="flex items-center gap-2 px-3 rounded-xl border" style={{ height: '44px', borderColor: '#E0EDE6' }}>
          ...
          <div className="flex justify-end">
            <button className="flex items-center gap-2 px-6 py-2.5 rounded-xl"
              style={{ background: '#2D6A4F', color: 'white', fontFamily: "'Be Vietnam Pro', sans-serif", fontWeight: 600, fontSize: '13.5px' }}>
