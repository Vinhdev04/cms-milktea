  ...
  return (
    <div style={{ fontFamily: "'Be Vietnam Pro', sans-serif" }}>
      ...
      <div className="mb-6">
        ...
        <p style={{ color: '#6B9080', fontSize: '13.5px' }}>Thứ Hai, 20 tháng 04 năm 2026 — Chào mừng trở lại, Admin!</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {statsCards.map((card, i) => (
          <div key={i} className="rounded-xl p-5 transition-all"
            style={{ background: '#FFFFFF', border: '0.5px solid #E0EDE6', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            <div className="flex items-start justify-between mb-3">
              ...
              <span className="text-xs px-2 py-1 rounded-full flex items-center gap-1"
                style={{
                  background: card.trend === 'up' ? '#DCFCE7' : card.trend === 'down' ? '#FEE2E2' : '#F3F4F6',
                  color: card.trend === 'up' ? '#166534' : card.trend === 'down' ? '#991B1B' : '#6B7280',
                  fontSize: '11px'
                }}>
            ...
            <div style={{ fontSize: '12.5px', color: '#6B9080', marginTop: '2px' }}>{card.title}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        ...
        <div className="lg:col-span-2 rounded-xl p-5"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
            <div>
              ...
              <p style={{ fontSize: '12px', color: '#6B9080' }}>Theo {chartPeriod === 'monthly' ? 'tháng năm 2025' : 'tuần này'}</p>
            </div>
            <div className="flex rounded-lg overflow-hidden border" style={{ borderColor: '#E0EDE6' }}>
              {(['weekly', 'monthly'] as const).map((p) => (
                <button key={p} onClick={() => setChartPeriod(p)}
                  className="px-3 py-1.5 transition-all text-xs"
                  style={{
                    background: chartPeriod === p ? '#2D6A4F' : 'white',
                    color: chartPeriod === p ? 'white' : '#6B9080',
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={chartData} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
              ...
              <Tooltip
                contentStyle={{ borderRadius: '10px', border: '1px solid #E0EDE6', boxShadow: '0 4px 16px rgba(0,0,0,0.08)', fontSize: '12px', fontFamily: "'Be Vietnam Pro', sans-serif" }}
        ...
        <div className="rounded-xl p-5"
          ...
          <p style={{ fontSize: '12px', color: '#6B9080', marginBottom: '12px' }}>Tháng 04/2026</p>
          <ResponsiveContainer width="100%" height={160}>
            <PieChart>
              ...
              <Tooltip contentStyle={{ borderRadius: '8px', border: '1px solid #E0EDE6', fontSize: '12px', fontFamily: "'Be Vietnam Pro', sans-serif" }}
            ...
            {categoryData.map((cat, i) => (
              <div key={i} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span style={{ fontSize: '12px', color: '#6B9080' }}>{cat.name}</span>
                </div>
                <span style={{ fontSize: '12px', fontWeight: 600, color: '#2D6A4F' }}>{cat.value}%</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="rounded-xl overflow-hidden"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: '#E0EDE6' }}>
          <div>
            ...
            <p style={{ fontSize: '12px', color: '#6B9080' }}>Hôm nay, 20/04/2026</p>
          </div>
          <a href="/orders" style={{ fontSize: '13px', color: '#2D6A4F', fontWeight: 600, textDecoration: 'none' }}
            ...
            <thead>
              <tr style={{ background: '#E8F5EC' }}>
                {['Mã đơn', 'Khách hàng', 'Chi nhánh', 'Món', 'Tổng tiền', 'TT Thanh toán', 'Trạng thái', ''].map((h, i) => (
                  <th key={i} className="text-left px-4 py-3" style={{ fontSize: '12px', fontWeight: 600, color: '#2D6A4F', whiteSpace: 'nowrap' }}>
                    {h}
                  </th>
                ))}
              </tr>
              ...
              {orders.slice(0, 6).map((order, i) => {
                ...
                return (
                  <tr key={order.id} className="border-b hover:bg-gray-50 transition-colors"
                    ...
                    <td className="px-4 py-3">
                      <span style={{ fontSize: '13px', color: '#6B9080' }}>{order.branch}</span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <span style={{ fontSize: '12px', color: '#6B9080' }}>{order.payment}</span>
                    </td>
                    <td className="px-4 py-3">
