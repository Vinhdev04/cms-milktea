...
export function UserMenu() {
  ...
  useEffect(() => {
    // Remove restriction for development so user can see it
    if (!window.driver) {
    ...
    const startTour = () => {
      const driverObj = window.driver.js.driver({
        ...
        steps: [
          { 
            element: '#tour-menu-title', 
            popover: { 
              title: '🎉 Thực đơn Chips 🧋', 
              description: 'Chào mừng bạn đến với thế giới trà sữa Chips! Hãy để chúng tôi hướng dẫn bạn cách chọn món ngon nhất.',
              side: "bottom", 
              align: 'start' 
            } 
          },
          { 
            element: '#tour-search', 
            popover: { 
              title: '🔍 Tìm món yêu thích', 
              description: 'Bạn có thể tìm kiếm theo tên món, hương vị hoặc topping mà mình yêu thích tại đây.',
              side: "bottom", 
              align: 'center' 
            } 
          },
          { 
            ...
            popover: { 
              title: '🍱 Phân loại món', 
              description: 'Dễ dàng lọc các món theo danh mục: Trà sữa, Trà trái cây, hay Cà phê để tìm kiếm nhanh hơn.',
              side: "bottom", 
              align: 'center' 
            } 
          },
          { 
            ...
            popover: { 
              title: '✨ Danh sách món ngon', 
              description: 'Tất cả các món đang sẵn sàng phục vụ bạn. Hãy chọn món và tùy chỉnh theo ý thích nhé!',
              side: "top", 
              align: 'center' 
            } 
          },
          { 
            ...
            popover: { 
              ...
              description: 'Kiểm tra lại các món đã chọn và tiến hành thanh toán tại đây.',
              side: "bottom", 
              align: 'end' 
            } 
          }
    ...
    const timer = setTimeout(startTour, 2000); // Wait 2s to ensure DOM is ready
  ...
  return (
    <div className="min-h-screen bg-[#FFFBF7] pb-32">
      ...
      <header className="sticky top-0 z-40 border-b border-orange-50 bg-white/80 pb-4 pt-6 backdrop-blur-xl" style={{ WebkitBackdropFilter: 'blur(20px)' }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center gap-2 anim-fade-up anim-fade-up-delay-1" id="tour-search">
      ...
      <nav className="sticky top-[148px] z-30 border-b border-orange-50 bg-white/60 py-4 backdrop-blur-md" style={{ WebkitBackdropFilter: 'blur(12px)' }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="flex gap-3 overflow-x-auto no-scrollbar py-2" id="tour-categories">
            <button
      ...
      <main className="mx-auto max-w-7xl px-4 pt-10 sm:px-6">
        <div className="grid gap-10 lg:grid-cols-[1fr_320px]">
          <div>
            ...
            <div className="grid gap-6 grid-cols-1 md:grid-cols-2 xl:grid-cols-3 stagger-children" id="tour-product-list">
          ...
          <aside className="hidden lg:block anim-slide-right">
            <div className="sticky top-[280px] space-y-6">
              ...
              <div className="rounded-[32px] bg-[#2D1606] p-6 text-white shadow-2xl overflow-hidden relative group card-interactive">
                 ...
                 <p className="mt-2 relative z-10 text-sm text-white/60">Đừng quên tích điểm Chips để đổi quà Gold Member.</p>
        ...
        onClick={() => {
          if (window.driver) {
             const d = window.driver.js.driver({
               showProgress: true,
               steps: [
                 { element: '#tour-menu-title', popover: { title: '🎉 Thực đơn Chips', description: 'Bắt đầu khám phá nhé!' } },
                 { element: '#tour-search', popover: { title: '🔍 Tìm kiếm', description: 'Tìm món tại đây.' } },
                 { element: '#tour-categories', popover: { title: '🍱 Danh mục', description: 'Lọc món theo loại.' } },
                 { element: '#tour-product-list', popover: { title: '✨ Món ngon', description: 'Chọn món bạn thích.' } },
                 { element: '#tour-cart', popover: { title: '🛒 Giỏ hàng', description: 'Kiểm tra và thanh toán.' } }
               ]
             });
             d.drive();
          }
        }}
        className="fixed bottom-32 right-6 z-50 flex h-12 w-12 items-center justify-center rounded-full bg-[#2D1606] text-white shadow-2xl transition-all duration-300 hover:bg-orange-500 hover:scale-110 active:scale-95"
        title="Hướng dẫn"
      >
